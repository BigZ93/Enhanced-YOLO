...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    ...
    partialResults *partResults, int dataExtraction)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    char **names = get_labels_custom(name_list, &names_size); //get_labels(name_list);          //---names has labels---

    if (dataExtraction == 1) {
        FILE *f;
        f = fopen("names.csv", "w+");
        fprintf(f, "class,name\n");
    ...
    while (1) {
        ...
        network_predict(net, X);                //---net has predictions---
        //network_predict_image(&net, im); letterbox = 1;
        printf("%s: Predicted in %lf milli-seconds.\n", input, ((double)get_time_point() - time) / 1000);
        //printf("%s: Predicted in %f seconds.\n", input, (what_time_is_it_now()-time));
        
        int nboxes = 0;
        detection *dets = get_network_boxes(&net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes, letter_box, partResults);      //---dets has predictions too, added partResults---
        //printf("\nimg w: %d h: %d\n", im.w, im.h);
        //printf("in test_detector I x: %f y: %f\n", dets[0].bbox.x*im.w, dets[0].bbox.y*im.h);    //---coordinates are divided by image dimensions and kept as float---
        if (nms) {
            if (l.nms_kind == DEFAULT_NMS) do_nms_sort(dets, nboxes, l.classes, nms);
            else diounms_sort(dets, nboxes, l.classes, nms, l.nms_kind, l.beta_nms);
        }
...
void run_detector(int argc, char **argv)
{
    ...
    partialResults *partResultsDummy = NULL;    //--------------------dummies----------------
    image *imageDummy = NULL;
    char **namesDumy = NULL;
    image **alphabetDummy = NULL;
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers, partResultsDummy, imageDummy, namesDumy, alphabetDummy, 0);
