...
int get_yolo_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative, detection *dets, int letter, partialResults *partResults, int yoloLayer)
{
    int prnt = 0;
    struct classCounter {
        int classType;
        ...
        int countsy;
        float probability;
        struct classCounter *next;
    };
    struct classCounter *classCount = NULL;
    classCount = malloc(sizeof(struct classCounter));
    struct classCounter *head = classCount;
    struct classCounter *head2 = classCount;
    struct classCounter *head3 = classCount;
    struct classCounter *head4 = classCount;
    ...
    int count = 0;      //---count counts recognized boxes (but before merging them into a single one)---
    int count2 = 0;
    for (i = 0; i < l.w*l.h; ++i){
        int row = i / l.w;
        int col = i % l.w;
        for(n = 0; n < l.n; ++n){
            ...
            if (objectness > thresh) {
                ...
                classCount->probability = 0;
                classCount->elementsFound = count2;
                classCount->countsy = count;
                ...
                for (j = 0; j < l.classes; ++j) {
                    ...
                    if (prnt == 1) {
                        if (predictions[class_index] > thresh) {
                            printf("   %d: prob:%f pred:%f\n", j, prob, predictions[class_index]);
                        }
                    }

                    dets[count].prob[j] = (prob > thresh) ? prob : 0;       //---here are probabilities---

                    if (dets[count].prob[j] != 0) {
                        printf("   count2 %d count %d class %d probability %f\n", count2, count, j, prob);
                        classCount->classType = j;
                        classCount->probability = prob;
                        classCount->elementsFound = count2;
                        classCount->countsy = count;
                        classCount->next = malloc(sizeof(struct classCounter));
                        classCount = classCount->next;
                        classCount->next = NULL;
                        ++count2;
                    }
                }
                if (classCount->classType == -1) {
                    ++count2;
                    classCount->next = malloc(sizeof(struct classCounter));
                    classCount = classCount->next;
                    classCount->next = NULL;
    ...
    partResults[yoloLayer].probabilities = calloc(count2, sizeof(float));
    partResults[yoloLayer].classes = calloc(count2, sizeof(int));
    partResults[yoloLayer].hits = calloc(count2, sizeof(int));
    partResults[yoloLayer].numberOfProbabilities = count2;
    partResults[yoloLayer].totalNumberOfClasses = l.classes;
    if (head != NULL && count2 > 0) {
        for (int i = 0; i < count2;i++) {
            partResults[yoloLayer].classes[i] = head->classType;
            partResults[yoloLayer].hits[i] = head->elementsFound;
            partResults[yoloLayer].probabilities[i] = head->probability;
            head = head->next;
        }
    }
    if (prnt == 1) {
        printf("\n");
        for (int i = 0; i < count2; i++) {
            printf("%d ", partResults[yoloLayer].classes[i]);
        }
        printf("\n");
        for (int i = 0; i < count2; i++) {
            printf("%f ", partResults[yoloLayer].probabilities[i]);
        }
    }
    correct_yolo_boxes(dets, count, w, h, netw, neth, relative, letter);    //---boxes can be extracted from here---
    partResults[yoloLayer].bbox = calloc(count2, sizeof(box));
    for (int i = 0; i < count2; i++) {
        if (prnt == 1) {
            printf("count2 %d   count %d   class %d\n", head3->elementsFound, head3->countsy, head3->classType);
        }
        head3 = head3->next;
    }
    if (head2 != NULL && count2 > 0) {
        for (int i = 0; i < count2; i++) {
            if (prnt == 1) {
                printf("w get_yolo_detections x: %f y: %f\n", dets[head2->countsy].bbox.x, dets[head2->countsy].bbox.y);    //---good to multiply by image size---
            }
            if (partResults[yoloLayer].classes[i] == -1) {
                box b;
                b.x = 0;
                b.y = 0;
                b.h = 0;
                b.w = 0;
                partResults[yoloLayer].bbox[i] = b;
            }
            else {
                partResults[yoloLayer].bbox[i] = dets[head2->countsy].bbox;
            }
            partResults[yoloLayer].hits[i] = head2->countsy;
            head2 = head2->next;
        }
    }
    partResults[yoloLayer].numberOfCells = count;
    if (prnt == 1) {
        for (int i = 0;i < count2; i++) {
            printf("count2 %d   count %d   class %d   ", i, partResults[yoloLayer].hits[i], partResults[yoloLayer].classes[i]);
            printf("prob: %f   x: %f y: %f\n", partResults[yoloLayer].probabilities[i], partResults[yoloLayer].bbox[i].x, partResults[yoloLayer].bbox[i].y);    //---good to multiply by image size---
        }
    }
    return count;
}
