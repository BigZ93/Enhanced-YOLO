...
void fill_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets, int letter, partialResults *partResults)
{
    int prev_classes = -1;
    int j;
    int yoloLayer = 0;
    for (j = 0; j < net->n; ++j) {
        ...
        if (l.type == YOLO) {
            int count = get_yolo_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter, partResults, yoloLayer);
            dets += count;
...
detection *get_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, int *num, int letter, partialResults *partResults)
{
    detection *dets = make_network_boxes(net, thresh, num);
    fill_network_boxes(net, w, h, thresh, hier, map, relative, dets, letter, partResults);
    return dets;
}
